...
def pipelineBeingGeneratedName = "MTV_Spinnaker_cENM_Pipeline_Upgrade_Updater"
...
pipelineJob(pipelineBeingGeneratedName) {
    ...
    description(commonSteps.defaultJobDescription(pipelineBeingGeneratedName, '<p>This job is used to update the MTV cENM UG spinnaker pipeline</p>'))

    concurrentBuild(allowConcurrentBuild = true)

    parameters {
        stringParam('SLAVE', "${TE_DOCKER_SLAVES}", 'Slave to run pipeline against')
        stringParam(commonParams.spinnakerPipelineName('fem 4s11: MTV cENM UG Pipeline'))
    ...
    definition {
        cpsScm {
            ...
            scriptPath("Jenkinsfiles/spinnaker_pipelines/cenm/upgrade/mtv/spinnaker_cenm_pipeline_upgrade_updater/Jenkinsfile")
        }
    }
}
