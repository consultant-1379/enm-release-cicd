...
def pipelineBeingGeneratedName = "Spinnaker_cENM_Restore_Pipeline_Updater"
...
pipelineJob(pipelineBeingGeneratedName) {
    ...
    description(commonSteps.defaultJobDescription(pipelineBeingGeneratedName, '<p>This job is used to update cENM Restore spinnaker pipeline</p>'))

    concurrentBuild(allowConcurrentBuild = true)

    parameters {
        stringParam('SLAVE', "${OPENSTACK_SLAVES}", 'Slave to run pipeline against')
        stringParam(commonParams.spinnakerPipelineName('fem 4s11: cENM_Restore Pipeline'))
    ...
    definition {
        cpsScm {
            ...
            scriptPath("Jenkinsfiles/spinnaker_pipelines/cenm/restore/spinnaker_cenm_pipeline_restore_updater/Jenkinsfile")
        }
    }
}