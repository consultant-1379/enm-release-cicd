...
def pipelineBeingGeneratedName = "cENM_Send_Confidence_Level_To_Build_Log"
...
pipelineJob(pipelineBeingGeneratedName) {
    ...
    description(commonSteps.defaultJobDescription(pipelineBeingGeneratedName,
        '<p>This job communicates with the cENM CI Portal Build log to send cENM confidence levels via REST.</p>'))
    ...
    parameters {
        ...
        choiceParam('usecase', ['cENM-Deploy-II-Charts', 'cENM-Deploy-II-CSAR', 'cENM-Deploy-UG-Charts', 'cENM-Deploy-II-CSAR-Lite',
                                'cENM-Deploy-UG-CSAR', 'cENM-Deploy-UG-CSAR-Lite', 'cENM-Deploy-II-Charts-SI', 'cENM-Deploy-Rollback-Charts',
                                'cENM-Deploy-Rollback-CSAR','cENM-Deploy-Rollback-CSAR-Lite'], 'The name of the confidence level i.e., the primary usecase (II/UG/ROLLBACK) to send to the cENM build log')
        ...
        choiceParam('build_log_action', ['create', 'update', 'final'], '''Please select: <br>
                                          1) Create -> If it is a new entry in build log (II/UG/ROLLBACK), <br>
                                          2) Update -> To update an existing entry in build log''')
        choiceParam('confidence_level', ['', 'cENM-RFA', 'cENM-Build-Integration-Charts', 'cENM-Build-CSAR',
                                         'cENM-UG-Availability', 'UG-Performance', 'cENM-UI-Link-Launcher',
                                         'cENM-RFA-Rerunner', 'cENM-RFA-Staging'], 'Please select the Confidence Level which needs to be updated for the select Usecase')
    ...
    definition {
        cpsScm {
            ...
            scriptPath("Jenkinsfiles/child_jobs/cenm_send_confidence_level_to_build_log/Jenkinsfile")
        }
    }
}
