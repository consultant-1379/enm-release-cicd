...
def jobBeingGeneratedName = "cENM_RFA250_Staging"
...
job(jobBeingGeneratedName) {
    steps {
        ...
        description(commonSteps.defaultJobDescription(jobBeingGeneratedName, '''<p>This job is used to execute the RFA 250 Staging test suites against cENM environments</p>
<ul>
  <li>Staging testware is used to ensure stabilise and soak those tests for a period of a sprint.</li>
</ul>'''))
...
sh MTELoopScripts/pipeline_scripts/rfa_250_setup.sh "${WORKSPACE}/parameters.properties" || exit 1''')
        ...
        buildDescription('', '<b>Test Phase = ${test_phase}<br>Product Set Version = ${product_set_version}<br>ENM ISO = ${enm_iso_version}<br>cENM Product Set Version = ${cenm_product_set_version}<br>Central CSV Version = ${central_csv_version}<br>TAF Scheduler Version = ${taf_scheduler_version}<br>TAF Version = ${taf_version}</b>')
        ...
        baselineDefinedMessageDispatcher {
            ...
            schedule {
                name('enm_schedule_STAGING_RFA250_svc_scp_evt.xml')
